...
gw2_combat_movement.combatmovement = {
	...
	range = false,   -- "moving into combat range"
	allowed = true,  
}
...
function gw2_combat_movement:DoCombatMovement(target)
	...
	if(TimeSince(self.previoushealth.time) > 1000) then
		...
		self.previoushealth.health = ml_global_information.Player_Health.percent
	end

	if (domovement and target.distance <= fightdistance and ml_global_information.Player_OnMesh) then -- and ml_global_information.Player_Health.percent < 99
		...
		if (not isimmobilized) then		
			gw2_combat_movement:_DoMovement(target, fightdistance)
		else
...
function gw2_combat_movement:_DoMovement(target, fightdistance)

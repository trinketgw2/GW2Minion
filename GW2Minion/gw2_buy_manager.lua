...
function gw2_buy_manager.ModuleInit()
   ...
   end, tooltip = "Click to open \"Buy Manager\" window.", texture = GetStartupPath() .. "\\GUI\\UI_Textures\\buy.png" }, "GW2MINION##MENU_HEADER")
...
function gw2_buy_manager.mainWindow.Draw(event, ticks)
   if (gw2_buy_manager.mainWindow.open) then
      ...
      if (gw2_buy_manager.mainWindow.visible) then
         ...
         if (GUI:TreeNode(GetString("salvagekits"))) then
            -- create the tree, only pop inside if.
            -- Crude Kits.
         ...
         -->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
         GUI:SetNextTreeNodeOpened(true, GUI.SetCond_Appearing) -- open the tree *BUG* conditions not working as expected.
         if (GUI:TreeNode(GetString("gatheringtools"))) then
            -- create the tree, only pop inside if.
            -- Tool Type
            GUI:AlignFirstTextHeightToWidgets()
...
function gw2_buy_manager.setMapAvailablity(salvageItems, gatheringItems)
   ...
   for _, item in pairs(salvageItems) do
      for key = 1, 5 do
         if (item.itemID == gw2_buy_manager.tools.salvage[key]) then
            gw2_buy_manager.availableOnMap.salvage[key] = true
         end
      end
   end
   ...
   for _, item in pairs(gatheringItems) do
      for key = 1, 6 do
         if (item.itemID == gw2_buy_manager.tools.foraging[key]) then
            gw2_buy_manager.availableOnMap.gathering[key] = true
         end
      end
   end
end
...
function gw2_buy_manager.GetNeededSalvageKitList()
   local neededKits = {}
   if (tonumber(gw2_buy_manager.crudeKit) > 0 and gw2_buy_manager.availableOnMap.salvage[1]) then
      neededKits[gw2_buy_manager.tools.salvage[1]] = tonumber(gw2_buy_manager.crudeKit)
   end
   if (tonumber(gw2_buy_manager.basicKit) > 0 and gw2_buy_manager.availableOnMap.salvage[2]) then
      neededKits[gw2_buy_manager.tools.salvage[2]] = tonumber(gw2_buy_manager.basicKit)
   end
   if (tonumber(gw2_buy_manager.fineKit) > 0 and gw2_buy_manager.availableOnMap.salvage[3]) then
      neededKits[gw2_buy_manager.tools.salvage[3]] = tonumber(gw2_buy_manager.fineKit)
   end
   if (tonumber(gw2_buy_manager.journeymanKit) > 0 and gw2_buy_manager.availableOnMap.salvage[4]) then
      neededKits[gw2_buy_manager.tools.salvage[4]] = tonumber(gw2_buy_manager.journeymanKit)
   end
   if (tonumber(gw2_buy_manager.masterKit) > 0 and gw2_buy_manager.availableOnMap.salvage[5]) then
      neededKits[gw2_buy_manager.tools.salvage[5]] = tonumber(gw2_buy_manager.masterKit)
   end

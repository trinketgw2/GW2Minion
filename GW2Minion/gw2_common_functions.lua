...
function gw2_common_functions.BufflistHasBuffs(bufflist, buffIDs)
   ...
   if (table.valid(bufflist) == false) then
      return false
   end
...
function gw2_common_functions.GetBestCharacterTarget(maxrange)
   ...
   if (range < 200) then
      range = 750
   end -- extend search range a bit for melee chars
   ...
   if (target == nil) then
      target = gw2_common_functions.GetCharacterTargetExtended("onmesh," .. hostileCheck .. "nearest,los,maxdistance=" .. tostring(range))
   end

   if (target == nil) then
      target = gw2_common_functions.GetCharacterTargetExtended("aggro,onmesh,nearest")
   end

   if (target == nil) then
      target = gw2_common_functions.GetCharacterTargetExtended("onmesh," .. hostileCheck .. "nearest")
   end

   if (target == nil and not Settings.GW2Minion.ignoreyellowmobs) then
      target = gw2_common_functions.GetCharacterTargetExtended("onmesh,nearest,los,maxlevel=15")
   end

   if (target == nil and not Settings.GW2Minion.ignoreyellowmobs) then
      target = gw2_common_functions.GetCharacterTargetExtended("onmesh,nearest,maxlevel=15")
   end
...
function gw2_common_functions.GetBestCharacterTargetForAssist()
   ...
   if (target == nil) then
      target = gw2_common_functions.GetCharacterTargetExtended("maxdistance=" .. tostring(ml_global_information.AttackRange) .. hostileCheck .. ",los", GW2.HEALTHSTATE.Downed)
   end
   -- Try to get Enemy without los in range
   if (target == nil) then
      target = gw2_common_functions.GetCharacterTargetExtended("maxdistance=" .. tostring(ml_global_information.AttackRange) .. hostileCheck)
   end
...
function gw2_common_functions.GetBestAggroTarget(healthstate)
   ...
   if (range > 1000) then
      range = 1000
   end -- limit search range a bit for ranged chars
   ...
   if (not target) then
      target = gw2_common_functions.GetCharacterTargetExtended("player,onmesh,nearest,maxdistance=" .. tostring(range * 2), healthstate)
   end

   -- Try to get Aggro Enemy with los in range first
   if (not target) then
      target = gw2_common_functions.GetCharacterTargetExtended("aggro,onmesh,lowesthealth,los,maxdistance=" .. tostring(range), healthstate)
   end
         ...
         if (table.valid(target)) then
            if ((target.alive or target.downed) and target.isreachable) then
               ...
               if (not target.isplayer) then
                  -- don't blacklist players which are dead for 90sec..stupid in spvp WHY THE FUCK IS PVP USING THE PVE EVENT TARGET FUNCTION
                  gw2_blacklistmanager.AddBlacklistEntry(GetString("Temporary Combat"), target.id, target.name, 5000)
...
function gw2_common_functions.GetCharacterTargetExtended(filterstring, healthstate)
   ...
   if (gw2_common_functions.useCustomSMFilterSettings) then
      ...
      end      -- Attackmode for Assist : 1 = Everything, 2 = Players
      if (Settings.GW2Minion.smtargetmode == 2) then
         filterstring = filterstring .. ",lowesthealth"
      end
      if (Settings.GW2Minion.smtargetmode == 3) then
         filterstring = filterstring .. ",nearest"
      end
      if (Settings.GW2Minion.smtargetmode == 4) then
         filterstring = filterstring .. ",clustered=600"
      end
   end
...
function gw2_common_functions.handleConversation(result)
   if (Player:IsConversationOpen() and (Inventory:IsVendorOpened() == false or result == "repair") and string.valid(result)) then
      ...
      if (options) then
         ...
         for index = 0, #options do
            -- this is needed because NPCs often have the GW2.CONVERSATIONOPTIONS.Repair ID although they are offering vendoring
            local conversation = options[index]
   ...
   if (ml_global_information.Player_Health.percent < Settings.GW2Minion.evadehpthreshold and ml_global_information.Player_Endurance >= 50) then
      ...
      if (type(direction) ~= "number") then
         ...
         if (not table.valid(aggroTargets)) then
            -- TODO: only check this if there can be hostile players. -- and PvPManager:IsInMatch()) then -- needa similar wvwvw check.
      ...
      if ((type(direction) == "number" and direction ~= 3) or evadeTarget) then
         -- silly fix, but SM uses direction 3 to evade towards a target, while unstuck uses 4 and 5 to evade left n right. But we want it only to evade when being attacked
...
function gw2_common_functions.getDegreeDiffTargets(targetIDA, targetIDB)
   -- gw2_common_functions.getDegreeDiffTargets(targetID,Player.id) player relative to target.
   ...
   if (table.valid(targetA) and table.valid(targetB)) then
      ...
      if (targetA.isGadget or targetB.isGadget) then
         diffDegree = diffDegree - 90
      end
      if (diffDegree < 0) then
...
function gw2_common_functions.GetRandomPoint()
   ...
   local function _validpos(pos)
      ...
      for i, existing in ipairs(randompointsused) do
         ...
         if (TimeSince(existing.time) > maxblacklistduration) then
            table.remove(randompointsused, i)
         end
